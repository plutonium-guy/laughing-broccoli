...
# MAGIC %md
# MAGIC # Collection Risk Pipeline — Unified View Version
# MAGIC
# MAGIC Replaces multi-table SAP joins with ONE pre-joined view.
# MAGIC View grain = (customer_id, invoice_id, line_item) with denormalized
# MAGIC dunning + P2P + dispute + customer master info already attached.
# MAGIC
# MAGIC Output schema matches original training_df / predictions_df so
# MAGIC the downstream model + scoring code keeps working unchanged.
...
# CONFIG
# =========================================================
UNIFIED_VIEW = "f_erp_glide_o2c_12.vw_invoice_unified"   # <-- replace with real view name
...
def load_base_data():
    """
    Single source — unified view already has joins done.
    Filter to lookback + ensure valid baseline_date.
    """
    df = spark.table(UNIFIED_VIEW).select(
        ...
        # View already scoped to 2 years upstream — no baseline lookback needed
...
# =========================================================
# 2. DUE DATE — already on the view, but derive if null
# =========================================================
def ensure_due_date(df):
    """View has due_date — if any row null, fall back to payment-terms tiers."""
...
# =========================================================
# 3. COUNTRY FILTER (column already on view — no join needed)
# =========================================================
...
def build_invoice_level(df):
    ...
    dunning/p2p/credit columns (same across all line items of an invoice).
    `source` tracked at invoice level — if ANY line item is BSAD with
    clearing_date set, invoice is considered cleared.
    """
...
# =========================================================
# 8. DUNNING FEATURES (denormalized on view — aggregate to customer)
# =========================================================
def compute_dunning_features(invoice_df, snapshots):
    """
    View already has dunning_level + dunning_count per invoice.
...
# =========================================================
# 9. P2P FEATURES (denormalized on view — aggregate to customer)
# =========================================================
def compute_p2p_features(invoice_df, snapshots):
    """
    View has one promise per invoice (latest). Aggregate per
...
# =========================================================
# 10. TARGET (forward 30d collection)
# =========================================================
def create_target(invoice_df, snapshots):
    ...
    future = inv.join(
        ...
        # AND must clear within snapshot..snapshot+30d (so it WAS BSAD eventually)
...
# =========================================================
# INFERENCE VERSION — snapshot = TODAY only
# =========================================================
def build_inference_dataset():
    """Single-snapshot version for production scoring."""
    print("Loading unified view for inference...")
    base = load_base_data()
    base = ensure_due_date(base)
    base = filter_by_countries(base, COUNTRIES)
    ...
    # Spine = distinct customers active today, snapshot = TODAY
    spine = inv.select("customer_id").distinct() \
        .withColumn("snapshot_date", F.lit(TODAY).cast("date"))
