...
def load_base_data(date_val):
    df = spark.table(UNIFIED_VIEW).select(
        ...
        F.col("payment_terms"),
        F.date_format(F.col("due_date"), "yyyy-MM-dd").alias("due_date"),
        ...
        F.date_format(F.col("last_dunned_date"), "yyyyMMdd").alias("last_dunned_date"),
        F.col("dunning_count").cast("int"),
        # P2P (denormalized)
        F.col("fin_promised_amt").cast("double"),
        F.col("fin_p2p_state").cast("int"),
        F.date_format(F.col("promise_create_dt"), "yyyyMMdd").alias("promise_create_dt"),
        F.date_format(F.col("promise_dt"), "yyyyMMdd").alias("promise_dt"),
        # credit
        F.col("risk_class"),
        F.col("credit_group"),
        F.col("credit_limit").cast("double"),
        # disputes
        F.date_format(F.col("dispute_create_date"), "yyyyMMdd").alias("dispute_create_date"),
...
MODEL_NAME = f"collection_risk_model_customer_v_1_{country_code}_optuna"
EXPERIMENT_PATH = f"/Workspace/Users/amiya.x.mandal@gsk.com/APEC/exp/collection_risk_model_customer_v_1_{country_code}"
N_TRIALS = 300
CV_SPLITS = 4
LEAKAGE_GAP_DAYS = 30
