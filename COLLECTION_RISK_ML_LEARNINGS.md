...
| `sample_data_and_test_v3.py` | LOCAL integration test (NOT a Databricks notebook). Generates a synthetic feature table (data_pipeline_v3 schema: 13 countries/3 clusters, ~45 weekly snapshots, V2 features, ~22% positive) and runs BOTH stacks end-to-end — global + cluster (real `CalibratedSeedEnsemble`, tiered tune). Run `python sample_data_and_test_v3.py`; sample CSVs → `/tmp/sample_data_v3/`. Validated 2026-06-14: both pass; the small tier lifts a small cluster's pr_auc 0.535→0.740 vs fixed. NOTE: panel needs enough weeks (~45) so the 30-day-embargo CV forms ≥2 folds — too few weeks and everything degrades to the fixed-param fallback. |
| `train_all_clusters_v3.py` | V3 cluster trainer — **one model per cluster + routing table**. Now **reads `collection_ml_features_train_v3`** (data_pipeline_v3) instead of inline PART A (removed 2026-06-12); PART B = the four levers (monotone + **tiered** Optuna walk-forward + calibration + seed-ensemble), per-country thresholds, registers `collection_risk_model_cluster_v3_<cluster>`, writes routing to `..._v3_staging` (prod behind `WRITE_PROD_ROUTING`). |
