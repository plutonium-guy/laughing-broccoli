...
# MAGIC %md
# MAGIC # Collection Risk Model — End-to-End Training + Diagnostic Plots
# MAGIC
# MAGIC Self-contained pipeline:
# MAGIC 1. **Preprocessing** — load SAP tables, clean dates, derive due dates, aggregate line items
# MAGIC 2. **Snapshot generation** — weekly snapshots, deduplicated monthly
# MAGIC 3. **Feature engineering** — exposure, behavior, dunning, P2P (60+ features)
# MAGIC 4. **Target creation** — 30-day forward collection ratio → binary label
# MAGIC 5. **Optuna training** — Bayesian search, walk-forward CV, F2 threshold tuning
# MAGIC 6. **Diagnostic plots** — data exploration, Optuna search, evaluation, SHAP, learning curves, risk bands
# MAGIC 7. **MLflow logging** — params, metrics, model registry

# COMMAND ----------

# MAGIC %pip install optuna xgboost mlflow shap category-encoders seaborn plotly
...
# Training config
MODEL_NAME = "collection_risk_model_customer_v1_optuna"
EXPERIMENT_PATH = "/Users/nishant.x.guvvada@gsk.com/collection_risk_model_customer_v1"
...
TARGET_RECALL = 0.75   # recall floor — model must catch >=75% of high-risk
                       # threshold then picked to MAXIMIZE precision under this floor
                       # gives recall-leaning but balanced trade-off (was 0.85 = recall-extreme)
...
def load_base_data():
    """Load BSAD, validate dates, lookback filter."""
    ...
    df = bsad.filter(
        ...
        F.col("zbd3t"), F.col("zbd2t"), F.col("zbd1t"),
    )
...
def create_snapshots(invoice_df):
    ...
    base_snapshots = base_snapshots.withColumn(
        "month_bucket", F.date_format("snapshot_date", "yyyy-MM")
...
def load_dunning_features(snapshot_df):
    ...
    s = snapshot_df.select(F.col("customer_id").alias("s_customer_id"), "snapshot_date")
    ...
    return df.groupBy("s_customer_id", "snapshot_date").agg(*aggs).withColumn(
        "high_dunning_ratio",
...
def filter_by_countries(df, countries):
    customer_details = spark.table("hive_metastore.t_erp_ordertocash_rbp_csi.kna1").select(
        F.col("KUNNR").alias("customer_id"),
        F.col("LAND1").alias("country"),
        F.col("REGIO").alias("region"),
    )
    df = df.join(customer_details, on="customer_id", how="left")
    if not countries:
        return df
...
# =========================================================
# PART B — POST-PROCESSING (Spark → pandas)
# =========================================================
...
def build_time_series_splits(pdf, n_splits=4, gap_days=30):
    ...
    for i in range(1, n_splits + 1):
        ...
        val_end = dates[min(fold_size * i + fold_size, n - 1)]
...
def run_optuna_training(training_df):
    ...
    with mlflow.start_run(run_name="optuna_search") as parent_run:
        ...
        y_val_prob = final_model.predict_proba(dev_pdf.loc[last_val_idx, selected])[:, 1]
        y_val_true = dev_pdf.loc[last_val_idx, "target"]
...
def _native_shap_values(model, X_clean):
    """
    Bypass shap.TreeExplainer (broken with XGBoost 2.x base_score format).
    Use XGBoost's native pred_contribs=True which returns SHAP values
    directly from the booster. Last column is the bias term — drop it.
    """
    import xgboost as xgb_mod
    booster = model.get_booster()
    dmatrix = xgb_mod.DMatrix(X_clean, feature_names=list(X_clean.columns))
    contribs = booster.predict(dmatrix, pred_contribs=True)
    shap_values = contribs[:, :-1]   # drop bias
    bias = float(contribs[0, -1])    # for waterfall expected value
    return shap_values, bias


def plot_shap_summary(model, X_sample):
    X_clean = _coerce_numeric(X_sample)
    shap_values, _ = _native_shap_values(model, X_clean)
...
def plot_shap_waterfall(model, X_sample, row_idx=0):
    X_clean = _coerce_numeric(X_sample)
    shap_values, bias = _native_shap_values(model, X_clean.iloc[[row_idx]])

    exp = shap.Explanation(
        values=shap_values[0],
        base_values=bias,
        data=X_clean.iloc[row_idx].values,
...
output = run_optuna_training(training_df)

# Step 3 — all diagnostic plots
run_all_plots(training_df, output)
